...
public class ImageFragment extends Fragment implements OnItemSelectedListener {
  ...
  public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
    ...
    viewModel.getSelectedItem().observe(getViewLifecycleOwner(), (item) -> {
      ...
      if (item != selectedAnimals) {
        selectedAnimals = item;
        if (animals != null) {
          updateSelection();
  ...
  private void updateSelection() {
    spinner.setSelection(selectedAnimals);
    Picasso.get().load(animals.get(selectedAnimals).getUrl()).into(imageView);
  }
